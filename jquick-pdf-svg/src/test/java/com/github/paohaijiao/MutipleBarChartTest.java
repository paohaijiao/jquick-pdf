 ...
 * @since 2026/4/18
 */
public class MutipleBarChartTest {
    @Test
    public void testMutipleChar1() throws IOException {
        JMultiBarChartData data = new JMultiBarChartData();
        data.setTitleText("2024年各产品线季度销售额（万元）");
        data.setXAxisLabels(Arrays.asList("Q1", "Q2", "Q3", "Q4"));
        data.setXAxisTitle("季度");
        data.setYAxisTitle("销售额（万元）");
        // 产品A数据
        JMultiBarChartData.BarData productA = new JMultiBarChartData.BarData();
        productA.setLegendText("产品A");
        productA.setBarColor(new Color(84, 112, 198));
        productA.setValues(Arrays.asList(65.0, 78.0, 88.0, 95.0));
        // 产品B数据
        JMultiBarChartData.BarData productB = new JMultiBarChartData.BarData();
        productB.setLegendText("产品B");
        productB.setBarColor(new Color(250, 200, 88));
        productB.setValues(Arrays.asList(45.0, 62.0, 71.0, 85.0));
        // 产品C数据
        JMultiBarChartData.BarData productC = new JMultiBarChartData.BarData();
        productC.setLegendText("产品C");
        productC.setBarColor(new Color(238, 102, 102));
        productC.setValues(Arrays.asList(52.0, 58.0, 69.0, 82.0));
        data.setBarDataList(Arrays.asList(productA, productB, productC));
        JOption option = new JOption();
        option.setData(data);
        JMultiBarChartRenderer renderer = new JMultiBarChartRenderer();
        renderer.render(option, "d://test//multipleBar.svg");
    }
