...
- [Demo Gallery](#demo-gallery)
  - [Image index: image → test class → method](#image-index-image--test-class--method)
    ...
    - [37. Project logo](#37-project-logo--imagesjquick-logosvg)
    - [38. Banner](#38-banner--imagesbannersvg)
...
#### 37. Project logo — `images/jquick-logo.svg`

![jquick-logo](images/jquick-logo.svg)

**Test Class:** `com.github.paohaijiao.demo.creditreport.JQuickCreditReportTest` (module `jquick-pdfx`) | **Method:** `reportByContent()`

Shown in the header of this document; the same test method produces the report page above.
...
JQuickPdfFactory factory = new JQuickPdfFactory(config);
factory.bind("svg", new JAdaptor(svgReader).getRuleContent());
FileOutputStream fileOutputStream = new FileOutputStream(path + "test.pdf");
fileOutputStream.write(factory.executeContent(new JAdaptor(fileReader).getRuleContent()));
```

#### 38. Banner — `images/banner.svg`

![banner](images/banner.svg)

No test class in this repository produces this file: it ships inside the external `jquick-banner` artifact (the banner block used by the credit report demo), declared in [pom.xml](file:///d:/idea/jquick-pdf/pom.xml):
